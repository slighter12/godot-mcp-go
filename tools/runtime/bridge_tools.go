...
type bridgeMCPContext struct {
	SessionID               string `json:"session_id"`
	EditorSessionID         string `json:"editor_session_id"`
	RuntimeSessionID        string `json:"runtime_session_id"`
	RuntimeCommandSessionID string `json:"runtime_command_session_id"`
	SessionInitialized      bool   `json:"session_initialized"`
}

func editorContextSessionID(ctx bridgeMCPContext) string {
	if editorSessionID := strings.TrimSpace(ctx.EditorSessionID); editorSessionID != "" {
		return editorSessionID
	}
	return strings.TrimSpace(ctx.SessionID)
}

func runtimeContextSessionID(ctx bridgeMCPContext) string {
	if runtimeSessionID := strings.TrimSpace(ctx.RuntimeSessionID); runtimeSessionID != "" {
...
func (t *BridgeEditorSyncTool) Execute(args json.RawMessage) ([]byte, error) {
	var payload struct {
		...
		EditorSessionID string                       `json:"editor_session_id"`
		Context         bridgeMCPContext             `json:"_mcp"`
	}
	...
	if editorSessionID == "" {
		editorSessionID = editorContextSessionID(payload.Context)
	}
...
func (t *BridgeRuntimeRegisterTool) Execute(args json.RawMessage) ([]byte, error) {
	...
	if editorContextID == "" || !payload.Context.SessionInitialized {
		log.Printf("godot-mcp runtime register rejected: reason=editor_session_missing session_id=%q runtime_session_id=%q", strings.TrimSpace(payload.SessionID), runtimeSessionID)
	...
	if sessionID == "" {
		log.Printf("godot-mcp runtime register rejected: reason=game_session_missing runtime_session_id=%q", runtimeSessionID)
	...
	if !ok {
		log.Printf("godot-mcp runtime register rejected: reason=game_session_missing session_id=%q runtime_session_id=%q", sessionID, runtimeSessionID)
	...
	if !runtimebridge.DefaultGameSessionRegistry().MatchesLaunchToken(sessionID, payload.LaunchToken) {
		log.Printf("godot-mcp runtime register rejected: reason=launch_token_mismatch session_id=%q runtime_session_id=%q payload_launch_token_present=%t", sessionID, runtimeSessionID, strings.TrimSpace(payload.LaunchToken) != "")
	...
	editorSessionID := strings.TrimSpace(payload.EditorSessionID)
	if editorSessionID == "" {
		editorSessionID = editorContextID
	}
	if editorSessionID == "" {
...
func (t *BridgeRuntimeSnapshotPushTool) Execute(args json.RawMessage) ([]byte, error) {
	...
	if editorContextID == "" || !payload.Context.SessionInitialized {
		log.Printf("godot-mcp runtime snapshot rejected: reason=editor_session_missing session_id=%q runtime_session_id=%q", strings.TrimSpace(payload.SessionID), runtimeSessionID)
	...
	sessionID := strings.TrimSpace(payload.SessionID)
	if sessionID == "" {
		log.Printf("godot-mcp runtime snapshot rejected: reason=game_session_missing runtime_session_id=%q", runtimeSessionID)
		return nil, tooltypes.NewRuntimeInvalidParamsError("session_id is required", t.Name(), "game_session_missing", nil)
	}
	if !runtimebridge.DefaultGameSessionRegistry().RuntimeSessionMatches(sessionID, runtimeSessionID) {
		expectedRuntimeSessionID, _ := runtimebridge.DefaultGameSessionRegistry().RuntimeSessionID(sessionID)
		log.Printf("godot-mcp runtime snapshot rejected: reason=runtime_session_mismatch session_id=%q runtime_session_id=%q expected_runtime_session_id=%q", sessionID, runtimeSessionID, strings.TrimSpace(expectedRuntimeSessionID))
	...
	if !sessionBefore.HasSnapshot {
		log.Printf("godot-mcp runtime snapshot accepted: first_snapshot=true session_id=%q runtime_session_id=%q snapshot_id=%q frame=%d", sessionID, runtimeSessionID, strings.TrimSpace(payload.Snapshot.SnapshotID), payload.Snapshot.Frame)
	}
	if sessionBefore.HasSnapshot {
		log.Printf("godot-mcp runtime snapshot accepted: first_snapshot=false session_id=%q runtime_session_id=%q snapshot_id=%q frame=%d", sessionID, runtimeSessionID, strings.TrimSpace(payload.Snapshot.SnapshotID), payload.Snapshot.Frame)
	}
	return json.Marshal(map[string]any{
		"source":      "runtime",
		"synced":      true,
