...
func (t *ListOfferingsTool) Execute(args json.RawMessage) ([]byte, error) {
	result := map[string]any{"offerings": []map[string]any{{"name": "godot-mcp", "version": "0.1.0", "capabilities": map[string]any{"tools": map[string]any{"listChanged": true}, "resources": map[string]any{"subscribe": true, "listChanged": true}, "prompts": map[string]any{"listChanged": true}, "logging": map[string]any{}, "completions": map[string]any{}}, "serverInfo": map[string]any{"name": "godot-mcp-go", "version": "0.1.0"}}}}
