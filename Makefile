...
.PHONY: help test-go test-http-smoke test-http-delete inspector-pull test-inspector-docker test-all
...
test-http-smoke:
	...
	for i in $$(seq 1 80); do \
		if curl -sSf "http://$(SERVER_HOST):$(SERVER_PORT)/" >/dev/null 2>&1; then break; fi; \
		sleep 0.2; \
	done; \
...
test-all: test-go test-http-smoke test-http-delete test-inspector-docker
